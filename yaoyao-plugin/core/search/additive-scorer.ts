...
export interface AdditiveScoreResult {
  id: number | string;
  snippet: string;
...
export interface AdditiveScorerConfig {
  /** Entity boost weight (default: 0.5) */
  entityBoostWeight?: number;
  /** Semantic score: minimum threshold to pass filter (default: 0 — no filter) */
  semanticThreshold?: number;
  /** Enable BM25 keyword signal (default: true) */
  useBM25?: boolean;
  /** Enable entity boost signal (default: true) */
  useEntityBoost?: boolean;
  /** Top-K results to return (default: 10) */
  topK?: number;
}

const DEFAULT_CONFIG: Required<AdditiveScorerConfig> = {
...
export function additiveScoreAndRank(
  ...
  const bm25Scores = new Map<string | number, number>();
  if (cfg.useBM25) {
  ...
  if (hasEntity) maxPossible += cfg.entityBoostWeight;

  // 4. Score each candidate
  const scored: AdditiveScoreResult[] = [];
 ...
 */
export function formatAdditiveResults(results: AdditiveScoreResult[], query: string): string {
