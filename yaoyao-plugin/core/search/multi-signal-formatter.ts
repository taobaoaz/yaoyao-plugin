...
export function formatMultiSignalResults(results: MultiSignalResult[], query: string): string {
  if (results.length === 0) return '没有找到相关记忆。';

  const lines = [
    `## 搜索结果（多信号融合）`,
    `查询: ${query}`,
    `信号: ${Object.keys(signalLabels)
      .filter((k) => results.some((r) => r.signals[k as keyof typeof r.signals] !== undefined))
      .join(' + ')}`,
    '',
  ];
