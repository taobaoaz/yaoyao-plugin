...
import {
  stepInstallCheck,
  stepConfigValidation,
  stepCoreInit,
  stepManifest,
  stepScopeManager,
  stepCrossSessionRecovery,
  stepMigration,
  stepCleanupScheduler,
...
import {
  ...
  runInstallCheck,
  formatInstallCheck,
  detectScheduledResetRisks,
  formatResetRiskReport,
} from '../utils/check-barrel.ts';
...
export function bootstrapYaoyao(
  ...
  if (resetRisks.length > 0) {
    ...
    for (const risk of resetRisks) {
      const level =
        risk.severity === 'critical' ? 'error' : risk.severity === 'warning' ? 'warn' : 'info';
  ...
  const embedding =
    registry.service<ReturnType<typeof import('../utils/embedding.ts').createEmbeddingService>>(
      'embedding',
    );
  ...
  if (config.capture?.enabled !== false) {
    const capHandle = registerCaptureHook(
      api,
      store,
      storage,
      config,
      registry.isActive('verify'),
      scopeManager,
      llmResult?.client ?? null,
      audit,
      embedding,
    );
