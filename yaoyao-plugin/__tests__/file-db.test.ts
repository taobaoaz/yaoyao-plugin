...
describe('FileDB', () => {
  ...
  it('searches via FTS match syntax (basic substring)', () => {
    const { db, dir } = freshDB();
    db.prepare('INSERT INTO memory_meta (date, user_text, asst_text) VALUES (?, ?, ?)').run(
      '2026-05-17',
      'Hello World',
      'Test search content',
    );
  ...
  it('deletes via delete from memory_meta', () => {
    ...
    assert.strictEqual(
      Number((db.prepare('SELECT count(*) FROM memory_meta').get() as Record<string, unknown>).c),
      1,
    );
  ...
  it('close persists the index', () => {
    ...
    db1
      .prepare('INSERT INTO memory_meta (date, user_text, asst_text) VALUES (?, ?, ?)')
