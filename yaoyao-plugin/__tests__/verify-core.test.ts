...
import { detectSpeculative, detectCorrection, scoreEvidence } from '../core/verify/verify.ts';
...
describe('scoreEvidence', () => {
  ...
  it('returns partial for medium overlap', () => {
    const snippets = [{ snippet: '用户提到喜欢小动物', filename: '2026-05-10.md' }];
    const result = scoreEvidence('用户养猫', snippets);
    assert.strictEqual(result.verdict, 'partial');
  });

  it('detects contradiction on negation mismatch', () => {
    const snippets = [{ snippet: '用户明确表示不喜欢猫', filename: '2026-05-10.md' }];
  ...
  it('handles invalid snippets array', () => {
    const result = scoreEvidence(
      '测试',
      'not an array' as unknown as Array<{ snippet: string; filename: string }>,
    );
    assert.strictEqual(result.verdict, 'unconfirmed');
    assert.strictEqual(result.confidence, 0);
  });
  ...
  it('handles empty snippet strings', () => {
    const snippets = [{ snippet: '', filename: 'empty.md' }];
    const result = scoreEvidence('用户养猫', snippets);
