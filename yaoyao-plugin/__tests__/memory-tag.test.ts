...
describe('记忆标签系统 (DB 层)', { concurrency: 1 }, () => {
  ...
  it('按标签搜索', () => {
    const stmt = db.prepare(
      'SELECT t.memory_id, m.user_text, m.date ' +
        'FROM memory_tags t JOIN memory_meta m ON t.memory_id = m.id ' +
        'WHERE t.tag = ?',
    );
  ...
  it('移除标签', () => {
    ...
    del.run('工作');
    const count = db
      .prepare("SELECT COUNT(*) as c FROM memory_tags WHERE tag = '工作'")
      .get() as unknown;
  ...
  it('清理孤立标签', () => {
    ...
    const ghost = db
      .prepare("SELECT COUNT(*) as c FROM memory_tags WHERE tag = '幽灵'")
      .get() as unknown;
  ...
  it('不区分大小写', () => {
    ...
    const search = db
      .prepare('SELECT COUNT(*) as c FROM memory_tags WHERE tag = ?')
      .get('testtag') as unknown;
