...
describe('detectAtRisk', () => {
  ...
  it('flags old memories', () => {
    const memories = [{ keyword: 'old', filename: '2026-04-01.md', snippet: 'ancient' }];
    const boosts = [{ keyword: 'old', boostedAt: '2026-04-01T00:00:00Z' }];
    const atRisk = detectAtRisk(memories, boosts, [], 7);
  ...
  it('flags important memories even if never recalled', () => {
    const memories = [{ keyword: 'important', filename: 'x.md', snippet: 'valuable' }];
    const atRisk = detectAtRisk(
      memories,
      [],
      [{ keyword: 'important', taggedAt: '2026-05-01' }],
      7,
    );
    assert.strictEqual(atRisk.length, 1);
