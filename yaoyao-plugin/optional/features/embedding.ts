...
export const embeddingFeature: OptionalFeature<ReturnType<typeof createEmbeddingService>> = {
  ...
  init(api, config) {
    ...
    const sources = scanEmbeddingSources() as import('../../utils/env-scan.ts').EmbeddingSource[];
    const firstAuth = sources.find(
      (s: import('../../utils/env-scan.ts').EmbeddingSource) => s.hasAuth,
    );

    if (firstAuth) {
      api.logger.info?.(`[yaoyao-memory:embedding] Auto-detected source: ${firstAuth.label}`);
      api.logger.info?.(
        `[yaoyao-memory:embedding] To override, set embedding.enabled=true and configure apiKey/baseUrl/model explicitly.`,
      );
      const autoCfg = {
    ...
    return {
      ...
      service: null,
      message:
        'Embedding service disabled (set embedding.enabled=true + apiKey, or set env var OPENAI_API_KEY / DEEPSEEK_API_KEY / KIMI_API_KEY to auto-detect)',
    };
  },
};
