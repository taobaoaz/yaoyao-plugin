...
import { safeReadJson, safeExec } from './io-utils.ts';
...
export function createCronTool(api: OpenClawPluginApi): ToolRegistration {
  ...
  return {
    name: 'memory_cron',
    description:
      '管理 OpenClaw 定时任务（cron）：列出任务、检测冲突、给出优化建议。修改配置需人工确认后重启 gateway。',
    parameters: {
      type: 'object',
      properties: {
        action: {
          ...
          description:
            'list=列出任务, detect=检测冲突, suggest=优化建议, disable=生成禁用冲突任务的配置（需用户确认后手动应用）',
        },
        jobId: {
          type: 'string',
    ...
    execute: async (args: Record<string, unknown>) => {
      ...
      if (cfg && Array.isArray(cfg.cron)) {
        for (const job of cfg.cron as Array<Record<string, unknown>>) {
          jobs.push({
      ...
      if (action === 'suggest') {
        ...
        if (risks.some((r) => r.severity === 'critical')) {
          suggestions.push(
            '🔴 发现严重冲突：存在与 yaoyao-memory 冲突的定时任务，建议立即移除或修改',
          );
        }
        if (risks.some((r) => r.description.includes('整点'))) {
          suggestions.push(
            '🟡 部分任务设置在整点/半点执行，建议偏移随机分钟（如 :17、:42）避免系统拥堵',
          );
        }
        if (jobs.length === 0) {
        ...
        suggestions.push(
          '💡 建议：所有记忆相关的定时任务统一由 yaoyao-memory 管理，避免多系统冲突',
        );
      ...
      if (action === 'disable') {
        ...
        const modifiedJobs = jobs.map((j, idx) =>
        ...
        return {
          ...
          instruction:
            '将上述 modifiedConfig.cron 替换 openclaw.json 中的 cron 数组，然后重启 OpenClaw gateway 生效',
        };
      }
