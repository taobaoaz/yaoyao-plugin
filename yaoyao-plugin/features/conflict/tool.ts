...
export function createJudgeTool(db: DBBridge): ToolRegistration {
  return {
    ...
    execute: withErrorHandling(
      async (_id: string, params: Record<string, unknown>): Promise<ToolHandlerResult> => {
  ...
  return {
    ...
    execute: withErrorHandling(
      async (_id: string, params: Record<string, unknown>): Promise<ToolHandlerResult> => {
        ...
        if (action === 'list') {
          ...
          for (const row of rows) {
            try {
              ...
              const relations = meta.relations as
                | Array<{ relation: string; reason: string; judgedAt: string }>
                | undefined;
              if (!relations || relations.length === 0) continue;

              lines.push(
                `**记忆 ID ${row.id}** [${row.date}]`,
                `内容: ${(row.user_text ?? '').slice(0, 80)}...`,
              );
              for (const rel of relations) {
                lines.push(
                  `  ${formatRelation(rel.relation as ConflictRelation)} — ${rel.reason}（${rel.judgedAt.slice(0, 10)}）`,
                );
              }
              lines.push('');
            } catch (e: unknown) {
        ...
        const report =
          formatConflictCandidates(candidates) +
