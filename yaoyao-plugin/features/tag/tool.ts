...
function openDb(dbPath: string): UnifiedDB {
...
function getDb(store: MemoryStore, dbBridge?: DBBridge): { db: UnifiedDB; isOwned: boolean } {
  if (dbBridge) {
    ...
    parameters: {
      ...
      properties: {
        action: {
          ...
          description:
            '操作类型：add=添加标签，remove=移除标签，search=按标签搜索，popular=热门标签，clean=清理空标签',
          default: 'search',
        },
        tags: {
          type: 'array',
    ...
    execute: withErrorHandling(async (_id: string, params: Record<string, unknown>) => {
      ...
      try {
        ...
        if (action === 'add') {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `✅ 已给 ${matched} 条记忆添加标签 "${cleanTags.join('、')}"（${added} 次操作）`,
              },
            ],
          };
        }
          ...
          if (cleanTags.length > 0) {
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: `✅ 已移除标签 "${cleanTags.join('、')}"（${removed} 条记录）`,
                },
              ],
            };
          } else {
        ...
        const lines = results.map(
          (r) => `📝 [${r.date}] ${r.user_text || '(无)'} ${r.asst_text ? '| ' + r.asst_text : ''}`,
        );
        return {
          content: [
            {
