...
export function createForgetTool(store: MemoryStore, db: DBBridge): ToolRegistration {
  return {
    ...
    label: 'Memory Forget',
    description:
      'Delete memory entries matching a keyword or date. Use to remove outdated or incorrect memories.',
    parameters: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Keyword to match (deletes matching entries)',
          default: '',
        },
        date: {
          type: 'string',
          description: 'Date YYYY-MM-DD to delete an entire day',
          default: '',
        },
      },
      required: [],
      anyOf: [{ required: ['query'] }, { required: ['date'] }],
    },
    execute: withErrorHandling(async (_id: string, params: Record<string, unknown>) => {
      ...
      if (!query && !date) {
        return {
          content: [{ type: 'text', text: '请提供要删除的关键词（query）或日期（date）。' }],
        };
      }
      ...
      for (const f of files) {
        ...
        if (filtered.length !== lines.length) {
          ...
          try {
            ...
            console.error(
              `[yaoyao-memory:forget] Failed to write ${f.path}: ${writeErr instanceof Error ? writeErr.message : String(writeErr)}`,
            );
            // 回滚已修改的文件
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: `⚠️ 文件修改失败 ${f.path}: ${writeErr instanceof Error ? writeErr.message : String(writeErr)}，已中止，未删除索引。`,
                },
              ],
            };
          }
