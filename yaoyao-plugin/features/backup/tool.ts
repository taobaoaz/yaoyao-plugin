...
export function createBackupTool(store: MemoryStore): ToolRegistration {
  return {
    ...
    label: 'Memory Backup',
    description:
      '创建记忆数据的时间戳快照备份。支持全量备份和增量备份（仅备份修改过的文件），也可列出和清理备份。',
    parameters: {
      type: 'object',
      properties: {
        action: {
          ...
          description: 'create（默认）、list 列出备份、prune 清理（默认保留最近 10 个）',
          default: 'create',
        },
        mode: {
          type: 'string',
          enum: ['full', 'incremental'],
          description: '备份模式：full 全量、incremental 仅备份自上次备份以来新增/修改的文件',
          default: 'full',
        },
        keep: {
          type: 'number',
          description: 'prune 时保留的备份数量（默认 10，范围 1-100）',
          default: 10,
        },
      },
      required: [],
    ...
    execute: withErrorHandling(async (_id: string, params: Record<string, unknown>) => {
      ...
      const mode = String(params.mode || 'full') as 'full' | 'incremental';
      ...
      if (action === 'list') {
        ...
        return {
          content: [
            { type: 'text', text: `📋 备份列表 (共 ${backups.length} 个):\n\n${lines.join('\n')}` },
          ],
        };
      }
