...
export function createRemindTool(): ToolRegistration {
  return {
    ...
    label: 'Remind (Memory)',
    description:
      '创建记忆定时提醒。通过 openclaw cron 在指定时间触发，推送相关记忆到当前会话。支持 cron 表达式或自然语言描述时间。',
    parameters: {
      type: 'object',
      properties: {
        keyword: {
          type: 'string',
          description:
            '要提醒的记忆关键词，创建 cron 时会在推送中搜索相关记忆。留空则推送随机记忆。',
          default: '',
        },
        cron: {
          type: 'string',
          description:
            'cron 表达式，支持标准格式：秒 分 时 日 月 周。也可以用 human 模式传入自然语言描述（见 timeDescription）',
          default: '0 9 * * *',
        },
        timeDescription: {
          type: 'string',
          description:
            "自然语言时间描述（与 cron 二选一）。如：'每天早上8点'、'每周一上午9点'、'每天中午12点'。系统会将其转换为 cron 表达式。",
          default: '',
        },
        message: {
          type: 'string',
