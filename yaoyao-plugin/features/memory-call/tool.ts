...
export function createMemoryCallTool(
  ...
  return {
    ...
    execute: withErrorHandling(async (_id: string, params: Record<string, unknown>) => {
      ...
      if (params.intent) memoryCall.intent = String(params.intent) as import("../../utils/memory-call.ts").MemoryCallIntent;
      if (params.timeRange) {
        memoryCall.timeRange = { relative: String(params.timeRange) as import("../../utils/memory-call.ts").MemoryCallTimeRange["relative"] };
      }
