...
export function detectLegacy(
  config: YaoyaoMemoryConfig & Record<string, unknown>,
  workspaceDir: string,
): MigrationResult {
  const traces: string[] = [];

  // Check for old config keys
  if (getBool(config, 'psychology', false)) traces.push('config.psychology=true');
  if (getBool(config, 'intervention', false)) traces.push('config.intervention=true');
  if (getBool(config, 'moodTracking', false)) traces.push('config.moodTracking=true');
  ...
  for (const f of legacyFiles) {
    if (fs.existsSync(f)) traces.push(`legacy file: ${path.basename(f)}`);
  }
 ...
 */
export function cleanupOldSkills(logger: PluginLogger): void {
  const _skillsDir = path.join(os.homedir(), '.openclaw', 'workspace', 'skills');
  const _migratedDir = path.join(
    os.homedir(),
    '.openclaw',
    'extensions',
    'yaoyao-memory',
    ...
    try {
      ...
      if (jsonFiles.length > 0) {
        ...
        for (const jf of jsonFiles) {
          ...
          if (!fs.existsSync(dest)) fs.copyFileSync(jf, dest);
          migrated.push(rel);
        }
      }
      ...
      if (migrated.length > 0) {
        logger.info?.(
          `[yaoyao-memory] 已迁移 ${migrated.length} 个配置文件并从 ${name} 转移（${_migratedDir}），旧目录已清理`,
        );
      } else {
      ...
      logger.warn?.(
        `[yaoyao-memory] 清理旧 skill ${name} 失败: ${e instanceof Error ? (e instanceof Error ? e.message : String(e)) : String(e)}（无影响，继续启动）`,
      );
    }
  }
}
