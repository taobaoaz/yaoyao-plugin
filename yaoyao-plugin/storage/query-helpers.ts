...
export function getStats(db: UnifiedDB, vector: VectorStore | null): DBStats {
  try {
    const totalCount = db.prepare('SELECT COUNT(*) as c FROM memory_meta').get() as
      | { c: number }
      | undefined;
...
export function getAllTags(db: UnifiedDB): Array<{ tag: string; memory_id: number }> {
  try {
    const rows = db.prepare('SELECT tag, memory_id FROM memory_tags').all() as Array<{
      tag: string;
      memory_id: number;
    }>;
    return rows;
...
export function getAllMeta(db: UnifiedDB): Array<{ id: number; filename: string }> {
  try {
    const rows = db.prepare('SELECT id, date FROM memory_meta').all() as Array<{
      id: number;
      date: string;
    }>;
...
export function getConfig(db: UnifiedDB, key: string, defaultValue?: string | null): string | null {
  try {
    const row = db.prepare('SELECT value FROM memory_config WHERE key = ?').get(key) as
      | { value: string }
      | undefined;
...
export function incrementAccessCount(db: UnifiedDB, id: number): void {
  try {
    ...
    db.prepare('UPDATE memory_meta SET access_count = ?, tier = ? WHERE id = ?').run(
      newCount,
      newTier,
      id,
    );
...
export function getMemoryMeta(db: UnifiedDB, id: number): string | null {
  try {
    const row = db.prepare('SELECT meta FROM memory_meta WHERE id = ?').get(id) as
      | { meta: string | null }
      | undefined;
...
export function searchByMetaRelations(
  db: UnifiedDB,
  limit: number,
...
export function countTags(db: UnifiedDB): { total: number; unique: number } {
  try {
    const totalRow = db.prepare('SELECT COUNT(*) as c FROM memory_tags').get() as
      | { c: number }
      | undefined;
    const uniqueRow = db.prepare('SELECT COUNT(DISTINCT tag) as c FROM memory_tags').get() as
      | { c: number }
      | undefined;
...
export function getRecentRawMemories(
  db: UnifiedDB,
  limit: number,
