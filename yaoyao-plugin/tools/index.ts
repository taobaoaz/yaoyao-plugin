...
export function registerMemoryTools(
  ...
  if (registry?.isActive('cloud-sync')) {
    try {
      ...
      api.logger.warn?.(
        `[yaoyao-memory] Cloud sync tool skipped: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  try {
    ...
    api.logger.warn?.(
      `[yaoyao-memory] Unify tool skipped: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
  ...
  if (registry?.isActive('quality') ?? false) {
    try {
      ...
      api.logger.warn?.(
        `[yaoyao-memory] Quality tool skipped: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  if (registry?.isActive('retain') ?? false) {
    try {
      ...
      api.logger.warn?.(
        `[yaoyao-memory] Retain tool skipped: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  if (registry?.isActive('graph') ?? false) {
    try {
      ...
      api.logger.warn?.(
        `[yaoyao-memory] Graph tool skipped: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
  ...
  if (registry?.isActive('verify') ?? false) {
    try {
      ...
      api.logger.warn?.(
        `[yaoyao-memory] Verify tool skipped: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
