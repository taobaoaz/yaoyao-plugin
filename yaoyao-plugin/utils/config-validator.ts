...
import {
  ...
  isPositiveInt,
  inRange,
} from './config-validator-helpers.ts';
...
export function validateConfig(config: YaoyaoMemoryConfig): ConfigValidation[] {
  ...
  if (embed?.enabled === true) {
    if (!embed?.apiKey || embed.apiKey.length === 0) {
      pushError(
        ...
        'embedding.apiKey',
        'API key is missing',
        'Set embedding.apiKey to a valid API key',
      );
    }
    if (!embed?.baseUrl) {
      pushError(
        ...
        'embedding.baseUrl',
        'Base URL is missing',
        'Set embedding.baseUrl (e.g. https://api.openai.com/v1)',
      );
    } else if (!isValidUrl(embed.baseUrl)) {
      pushError(
        results,
        'embedding.baseUrl',
        `Not a valid URL: ${embed.baseUrl}`,
        'Check for typos (e.g. https://api.openai.com/v1)',
      );
    }
    if (!embed?.model) {
      pushWarn(
        ...
        'embedding.model',
        'Model name not set',
        'Set embedding.model (e.g. text-embedding-3-small, bge-m3)',
      );
    }
    if (
      embed?.dimensions !== undefined &&
      embed.dimensions !== null &&
      !isPositiveInt(embed.dimensions)
    ) {
      pushError(
        results,
        'embedding.dimensions',
        `Invalid dimensions: ${embed.dimensions}`,
        'Must be a positive integer (e.g. 128, 1024)',
      );
    }
    if (embed?.authType && embed.authType !== 'bearer' && embed.authType !== 'custom') {
      pushError(
        results,
        'embedding.authType',
        `Invalid authType: ${embed.authType}`,
        "Use 'bearer' or 'custom'",
      );
    }
    if (
      embed?.authType === 'custom' &&
      (!embed?.customHeaders || Object.keys(embed.customHeaders).length === 0)
    ) {
      pushWarn(
        results,
        'embedding.customHeaders',
        "authType is 'custom' but no customHeaders configured",
        "Add embedding.customHeaders (e.g. { x-api-key: '...' })",
      );
    }
  ...
  if (capture?.debounceMs !== undefined) {
    ...
    if (dm < 100) {
      pushWarn(
        results,
        'capture.debounceMs',
        `Very short debounce: ${dm}ms`,
        'Minimum 100ms to avoid excessive writes',
      );
    }
  }
  ...
  if (
    ...
    recall.strategy !== 'fts' &&
    recall.strategy !== 'vector'
  ) {
    pushError(
      results,
      'recall.strategy',
      `Invalid strategy: ${recall.strategy}`,
      "Use 'hybrid', 'fts', or 'vector'",
    );
  }

  // ── cloudSync ──
  const cloud = config.cloudSync as
    | { enabled?: boolean; provider?: string; endpoint?: string }
    | undefined;
  if (cloud?.enabled === true) {
    if (!cloud.provider || !['webdav', 's3', 'sftp', 'samba'].includes(cloud.provider)) {
      pushError(
        results,
        'cloudSync.provider',
        `Invalid provider: ${cloud.provider}`,
        "Use 'webdav', 's3', 'sftp', or 'samba'",
      );
    }
    if (!cloud.endpoint) {
  ...
  if (config.debug === true && config.verbose !== true) {
    pushInfo(
      results,
      'debug',
      'Debug mode enabled without verbose — some debug logs may be suppressed',
      'Set verbose: true to see full debug output',
    );
  }

  // ── hooks ──
  if (
    config.hooks?.commandNew?.enabled !== undefined &&
    typeof config.hooks.commandNew.enabled !== 'boolean'
  ) {
    pushWarn(
      results,
      'hooks.commandNew.enabled',
      `Invalid value: ${config.hooks.commandNew.enabled}`,
  ...
  if (mc?.enabled !== undefined && typeof mc.enabled !== 'boolean') {
    pushWarn(
      results,
      'memoryCall.enabled',
      `Invalid value: ${mc.enabled}`,
      'Must be true or false',
    );
  }

  // ── heartbeat ──
  const hb = config.hooks?.heartbeat as
    | { enabled?: unknown; maxResults?: unknown; minScore?: unknown; maxContextChars?: unknown }
    | undefined;
  if (hb?.enabled !== undefined && typeof hb.enabled !== 'boolean') {
    pushWarn(
      results,
      'hooks.heartbeat.enabled',
      `Invalid value: ${hb.enabled}`,
      'Must be true or false',
    );
  }
  if (hb?.maxResults !== undefined && typeof hb.maxResults !== 'number') {
    pushWarn(
      results,
      'hooks.heartbeat.maxResults',
      `Invalid value: ${hb.maxResults}`,
      'Must be a number',
    );
  }
  if (hb?.minScore !== undefined && typeof hb.minScore !== 'number') {
    pushWarn(
      results,
      'hooks.heartbeat.minScore',
      `Invalid value: ${hb.minScore}`,
      'Must be a number',
    );
  }
  if (hb?.maxContextChars !== undefined && typeof hb.maxContextChars !== 'number') {
    pushWarn(
      results,
      'hooks.heartbeat.maxContextChars',
      `Invalid value: ${hb.maxContextChars}`,
      'Must be a number',
    );
  }
