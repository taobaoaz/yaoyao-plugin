...
export function scanEmbeddingSources(): EmbeddingSource[] {
  ...
  try {
    ...
    if (fs.existsSync(globalConfigPath)) {
      const globalCfg = JSON.parse(fs.readFileSync(globalConfigPath, 'utf-8')) as Record<
        string,
        unknown
      >;
      const defaults = (globalCfg.agents as Record<string, unknown> | undefined)?.defaults as
    ...
    if (fs.existsSync(extDir)) {
      for (const entry of fs.readdirSync(extDir)) {
        ...
        if (fs.existsSync(pluginJsonPath)) {
          const manifest = JSON.parse(fs.readFileSync(pluginJsonPath, 'utf-8')) as Record<
            string,
            unknown
          >;
...
export function generateSetupGuide(
  ...
  currentEmbeddingEnabled: boolean,
): string[] {
  const lines: string[] = [];
  if (sources.length === 0) {
