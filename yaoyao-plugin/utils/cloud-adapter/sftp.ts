...
export class SFTPAdapter implements CloudAdapter {
  ...
  constructor(secrets: Secrets, opts?: AdapterFactoryOpts) {
    ...
    this.timeoutMs = Math.max(
      3_000,
      Math.min(120_000, opts?.timeoutMs ?? parseInt(secrets.SFTP_TIMEOUT_MS || '30000', 10)),
    );
  }
  ...
  private async runSftp(commands: string[]): Promise<{ ok: boolean; stdout: string }> {
    return new Promise((resolve) => {
      ...
      const child = execFile(
        cmd,
        cmdArgs,
        { timeout: this.timeoutMs, env: envOverride || process.env },
        (err, stdout, stderr) => {
          resolve(
            err ? { ok: false, stdout: stderr || err.message } : { ok: true, stdout: stdout || '' },
          );
        },
      );
      if (child.stdin) {
