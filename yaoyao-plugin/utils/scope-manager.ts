...
export class SimpleScopeManager {
  ...
  constructor(config: ScopeConfig = DEFAULT_SCOPE_CONFIG) {
    this.config = config;
  }
  ...
  grantAccess(agentId: string, scopes: string[]): void {
    this.config.agentAccess[agentId] = scopes;
  }

  /** Build a scope tag for memory metadata. */
  static buildScopeTag(agentId?: string, projectId?: string, userId?: string): string {
