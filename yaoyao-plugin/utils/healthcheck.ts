...
export function runHealthcheck(baseDir?: string): HealthResult {
  ...
  if (satisfiesVersion(nodeVersion, versions.nodeRange)) {
    checks.push({
      name: 'Node.js 版本',
      status: 'pass',
      message: `${nodeVersion} ✅ (要求 ${versions.nodeRange})`,
    });
  } else {
    checks.push({
      name: 'Node.js 版本',
      status: 'fail',
      message: `${nodeVersion} ❌`,
      detail: `要求 ${versions.nodeRange}。node:sqlite 从 Node 22 开始内置，当前版本无法加载数据库层。`,
    });
  }

  // 2. OpenClaw Gateway version — reads requirement from package.json
  let gatewayVer: string;
  try {
    ...
    if (satisfiesVersion(gatewayVer, versions.pluginApiRange)) {
      checks.push({
        ...
        status: 'pass',
        message: `${gatewayVer} ✅ (要求 ${versions.pluginApiRange})`,
      });
    } else {
      checks.push({
        name: 'OpenClaw Gateway',
        status: 'fail',
        message: `${gatewayVer} ❌`,
        detail: `要求 ${versions.pluginApiRange}。请升级 Gateway：npm install -g openclaw@latest`,
      });
    }
  } catch (e: unknown) {
    ...
    checks.push({
      ...
      status: 'warn',
      message: `无法检测 ⚠️`,
      detail: `插件要求 Gateway ${versions.pluginApiRange}。如功能异常请升级。`,
    });
  }
  ...
  if (dbCap.nodeSqliteAvailable) {
    ...
    checks.push({
      name: 'SQLite',
      status: 'warn',
      message: '无 SQLite 后端 ⚠️',
      detail:
        '将降级为 file-db 纯文件模式。记忆仍可保存，搜索降级为简单文本匹配。建议：npm install better-sqlite3，或升级到 Node 22+。',
    });
  }
  ...
  try {
    ...
    checks.push({
      ...
      status: 'warn',
      message: '未安装 ⚠️',
      detail: '向量搜索不可用，FTS5 纯文本搜索仍正常工作。如需向量搜索请 npm install sqlite-vec。',
    });
  }
  ...
  if (home && home !== '/' && fs.existsSync(home)) {
    ...
    checks.push({
      name: '用户主目录',
      status: 'warn',
      message: `${home || 'unknown'} ⚠️`,
      detail: '主目录异常。某些功能可能无法找到默认配置路径。建议显式设置 memoryDir。',
    });
  }
  ...
  try {
    ...
    checks.push({
      ...
      status: 'fail',
      message: `${memDir} ❌`,
      detail: `无法写入: ${err instanceof Error ? err.message : String(err)}。检查磁盘空间和权限。`,
    });
  }
  ...
  if (dbCap.nodeSqliteAvailable || dbCap.betterSqlite3Available) {
    ...
    try {
      ...
      const mode = testDb.prepare('PRAGMA journal_mode').get() as
      ...
      if (fs.existsSync(walFile2)) fs.unlinkSync(walFile2);
      ...
      if (String(mode?.journal_mode) === 'wal' || String(mode) === 'wal') {
        ...
        checks.push({
          ...
          message: 'WAL 不可用 ⚠️',
          detail:
            '文件系统/环境不支持 WAL（常见于 NFS、某些 Docker 卷）。数据库仍可用，但并发性能降低。',
        });
      }
    } catch (err: unknown) {
      checks.push({
  ...
  if (platform === 'linux' || platform === 'darwin') {
    ...
    checks.push({
      ...
      message: `Windows ⚠️`,
      detail:
        'Windows 支持实验性。云备份中的 Samba 功能依赖 Windows net use 命令。路径分隔符已自动处理。',
    });
  } else {
    checks.push({
      name: '操作系统',
      status: 'warn',
      message: `${platform} ⚠️`,
      detail: '未充分测试的平台。核心功能应可用，但可能有路径或编码问题。',
    });
  }
  ...
  if (locale.toLowerCase().includes('utf-8') || locale.toLowerCase().includes('utf8')) {
    ...
    checks.push({
      ...
      message: `${locale} ⚠️`,
      detail:
        '未检测到 UTF-8 locale。CJK 文本和 emoji 可能有编码问题。建议设置环境变量 LANG=en_US.UTF-8。',
    });
  }
    ...
    try {
      ...
      if (freeGb > 1) {
        ...
        checks.push({
          name: '磁盘空间',
          status: 'warn',
          message: `${freeGb.toFixed(1)} GB ⚠️`,
          detail: '磁盘空间不足。长期运行可能无法写入新记忆。',
        });
      }
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      ...
      checks.push({
        ...
        status: 'warn',
        message: '无法检测 ⚠️',
        detail: '无法获取磁盘空间信息。不影响功能，但建议确保有充足空间。',
      });
    }
  } else {
    checks.push({
      ...
      status: 'warn',
      message: '无法检测 ⚠️',
      detail: '当前 Node.js 版本不支持 statfsSync（需要 Node 22.7.0+）。建议升级 Node 版本。',
    });
  }
  ...
  try {
    ...
    checks.push({
      ...
      status: 'warn',
      message: '不可用 ⚠️',
      detail: 'git 命令未找到。自动迁移（yaoyao-soul 安装）将不可用，需手动安装。',
    });
  }
