...
export function runMemoryStatsCheck(
  ...
  try {
    ...
    const total = statsDb.prepare('SELECT COUNT(*) as c FROM memory_meta').get() as
      | { c: number }
      | undefined;
    const tierDist = statsDb
      .prepare('SELECT tier, COUNT(*) as c FROM memory_meta GROUP BY tier')
      .all() as { tier: string; c: number }[] | undefined;
    const avgAge = statsDb
      .prepare("SELECT AVG(julianday('now') - julianday(created_at)) as d FROM memory_meta")
