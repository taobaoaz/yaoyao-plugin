...
describe("LLMClient.chat (mocked fetch)", () => {
  ...
  it("returns parsed content from mocked API", async () => {
    ...
    const client = new LLMClient({ apiKey: "k", baseUrl: "https://api.openai.com/v1", model: "gpt-4o-mini" });
  ...
  it("includes auth Bearer header", async () => {
    ...
    globalThis.fetch = async (_input, init) => {
      ...
      return Promise.resolve({
        ok: true, status: 200,
        json: async () => ({ choices: [{ message: { content: "ok" } }], model: "m", usage: { prompt: 1, completion: 1, total: 2 } }),
      }) as Promise<Response>;
    };
    const client = new LLMClient({ apiKey: "sk-mykey", baseUrl: "https://api.openai.com/v1", model: "m" });
  ...
  it("throws on HTTP error", async () => {
    ...
    const client = new LLMClient({ apiKey: "bad", baseUrl: "https://api.openai.com/v1", model: "m" });
  ...
  it("sends system+user messages and returns content", async () => {
    globalThis.fetch = async () => ({
      ...
      json: async () => ({
        ...
        model: "m", usage: { prompt: 50, completion: 10, total: 60 },
      }),
    }) as Response;
