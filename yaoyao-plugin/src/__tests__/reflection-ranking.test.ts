...
describe("computeReflectionLogistic", () => {
  ...
  it("handles invalid inputs gracefully", () => {
    assert.strictEqual(computeReflectionLogistic(NaN, NaN, NaN), computeReflectionLogistic(0, 1, 0.1));
  });
});
...
describe("computeReflectionScore", () => {
  ...
  it("applies fallback penalty", () => {
    const normal = computeReflectionScore({
      ageDays: 0, midpointDays: 30, k: 0.1, baseWeight: 1, quality: 1, usedFallback: false,
    });
    const fallback = computeReflectionScore({
      ageDays: 0, midpointDays: 30, k: 0.1, baseWeight: 1, quality: 1, usedFallback: true,
    });
...
describe("normalizeReflectionLineForAggregation", () => {
  it("normalizes whitespace and case", () => {
    assert.strictEqual(
      normalizeReflectionLineForAggregation("  Hello   WORLD  "),
      "hello world"
    );
  });
});
