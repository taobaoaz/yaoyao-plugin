 ...
 */
import { pushError, pushWarn, pushInfo, isValidUrl, isPositiveInt, inRange, } from "./config-validator-helpers.js";
...
export function validateConfig(config) {
    ...
    // ── hooks ──
    if (config.hooks?.commandNew?.enabled !== undefined &&
    ...
    // ── heartbeat ──
    const hb = config.hooks?.heartbeat;
...
export function logValidationResults(results, logger) {
    for (const r of results) {
        const msg = `[yaoyao-memory:config] ${r.level.toUpperCase()} — ${r.field}: ${r.message}${r.suggestion ? ` (建议: ${r.suggestion})` : ''}`;
        if (r.level === 'error') {
            logger?.error?.(msg);
        }
