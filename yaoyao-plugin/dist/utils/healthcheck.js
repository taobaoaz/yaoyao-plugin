...
export function runHealthcheck(baseDir) {
    ...
    if (dbCap.nodeSqliteAvailable) {
        ...
        checks.push({
            ...
            detail: '将降级为 file-db 纯文件模式。记忆仍可保存，搜索降级为简单文本匹配。建议：npm install better-sqlite3，或升级到 Node 22+。',
        });
    }
        ...
        try {
            ...
            if (String(mode?.journal_mode) === 'wal' || String(mode) === 'wal') {
                ...
                checks.push({
                    ...
                    status: 'warn',
                    message: 'WAL 不可用 ⚠️',
                    detail: '文件系统/环境不支持 WAL（常见于 NFS、某些 Docker 卷）。数据库仍可用，但并发性能降低。',
                });
            }
        }
        catch (err) {
    ...
    if (platform === 'linux' || platform === 'darwin') {
        ...
        checks.push({
            ...
            status: 'warn',
            message: `Windows ⚠️`,
            detail: 'Windows 支持实验性。云备份中的 Samba 功能依赖 Windows net use 命令。路径分隔符已自动处理。',
        });
    }
    else {
        checks.push({
    ...
    if (locale.toLowerCase().includes('utf-8') || locale.toLowerCase().includes('utf8')) {
        ...
        checks.push({
            ...
            status: 'warn',
            message: `${locale} ⚠️`,
            detail: '未检测到 UTF-8 locale。CJK 文本和 emoji 可能有编码问题。建议设置环境变量 LANG=en_US.UTF-8。',
        });
    }
...
import { formatHealthcheck } from "./healthcheck-formatter.js";
import { runMemoryStatsCheck } from "./healthcheck-stats.js";
