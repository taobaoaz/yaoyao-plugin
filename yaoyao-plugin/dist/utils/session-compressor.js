 ...
 * 零外部依赖，纯本地
 */
import { TOOL_CALL_INDICATORS, CORRECTION_INDICATORS, DECISION_INDICATORS, ACKNOWLEDGMENT_PATTERNS, MEMORY_INTENT, MEMORY_INTENT_CJK, } from "./compressor-indicators.js";
import { compressTexts } from "./compressor-core.js";
export { compressTexts };
export function scoreText(text, index) {
...
export function estimateConversationValue(texts) {
    if (texts.length === 0)
        return 0;
    let value = 0;
    ...
    const hasCorrectionOrDecision = CORRECTION_INDICATORS.some((p) => p.test(joined)) ||
