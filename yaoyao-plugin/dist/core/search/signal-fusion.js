import { scoreBM25, buildBM25Index } from "./bm25.js";
...
function computeTemporalWeight(timestamp, halfLifeDays) {
    if (halfLifeDays <= 0 || !timestamp)
        return 1;
...
export function multiSignalFusion(query, ftsResults, vecResults, allResults, config = {}) {
    ...
    if (bm25Ranked.length > 0)
        lists.push(bm25Ranked);
    if (ftsRanked.length > 0)
        lists.push(ftsRanked);
