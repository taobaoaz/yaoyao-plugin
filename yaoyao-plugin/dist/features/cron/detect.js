...
export function detectCronRisks(jobs) {
    const risks = [];
    for (const job of jobs) {
        ...
        if (/^0\s+\d+\s+\*\s+\*\s+\*/.test(sched) ||
            /^0\s+\d+,\d+\s+\*\s+\*\s+\*/.test(sched) ||
            /^30\s+\d+\s+\*\s+\*\s+\*/.test(sched)) {
            risks.push({
                source: `cron job ${job.id || '(unknown)'}`,
                severity: 'info',
...
export function isConflictingJob(job) {
    const task = String(job.payload?.task || '').toLowerCase();
    return (task.includes('memory') ||
        task.includes('clean') ||
        task.includes('reset') ||
        task.includes('prune'));
}
