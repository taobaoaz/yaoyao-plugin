...
import { queryOpenClawDB, readDreams, getYaoyaoDbPath, getDailyFilesCount } from "./provider.js";
import { formatStatusReport, formatBackendsReport, formatCrossSearchResults } from "./formatter.js";
    ...
    return {
        ...
        execute: withErrorHandling(async (_id, params) => {
            ...
            if (action === 'status') {
                ...
                const ocDbSize = fs.existsSync(ocDbPath)
                    ? (fs.statSync(ocDbPath).size / 1024).toFixed(1)
                    : 'N/A';
                    ...
                    content: [
                        {
                            ...
                            text: formatStatusReport({
                                ...
                                yaoDbSize: fs.existsSync(yaoDbPath)
                                    ? (fs.statSync(yaoDbPath).size / 1024).toFixed(1)
                                    : 'N/A',
            ...
            if (action === 'backends') {
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: formatBackendsReport(files, dreams.events, yaoInfo),
                        },
                    ],
                };
            }
            if (action === 'search_all') {
                ...
                const ocResults = queryOpenClawDB('SELECT path, text FROM chunks_fts WHERE text MATCH ? LIMIT 5', [query]);
                const dreams = readDreams(memoryDir);
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: formatCrossSearchResults(query, ocResults, dreamMatches),
                        },
                    ],
                };
            }
            return {
                content: [{ type: 'text', text: '❌ 未知操作。支持: status, search_all, backends' }],
            };
        }),
    };
}
