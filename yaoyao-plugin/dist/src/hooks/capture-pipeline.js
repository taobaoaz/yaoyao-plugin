...
export function indexToFTS5(db, userContent, indexableAsst, date, meta, watermark, writeQueue) {
    ...
    if (indexableAsst.length > CHUNK_THRESHOLD) {
        ...
        for (let i = 0; i < chunkResult.chunks.length; i++) {
            const chunkMeta = { ...parsedMeta, chunkIndex: i + 1, totalChunks: chunkResult.chunkCount };
            const chunkMetaStr = Object.keys(chunkMeta).length > 0 ? JSON.stringify(chunkMeta) : undefined;
            if (writeQueue) {
