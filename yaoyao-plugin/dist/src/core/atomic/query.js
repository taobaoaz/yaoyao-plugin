 ...
 */
import { getAllFacts, findFactsByEntity } from "./store.js";
/** Find facts matching a natural language query (simple keyword matching). */
export function queryFacts(query) {
...
export function getEntityFacts(entity) {
    return findFactsByEntity(entity);
}
/** Build a simple knowledge graph from facts. */
export function buildFactGraph(facts) {
    const graph = {};
    for (const fact of facts) {
        if (!graph[fact.subject])
            graph[fact.subject] = [];
