 ...
 */
export function formatRelation(r) {
    const typeEmoji = {
        supersedes: "🔄",
        related: "🔗",
        causes: "➡️",
...
export function formatGraphPath(path) {
    if (path.length === 0)
        return "(no path)";
    return path.map(formatRelation).join(" → ");
}
export function formatGraphResults(results) {
    if (results.length === 0)
...
export function generateMermaidDiagram(relations) {
