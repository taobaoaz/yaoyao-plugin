...
export interface PersistHandlers {
  flushBatch: (
    tasks: Array<{ userContent: string; asstContent: string; date: string; meta?: string }>,
  ) => Promise<void>;
    ...
    flushBatch: async (tasks) => {
      ...
      for (const task of tasks) {
        try {
          ...
          api.logger.error?.(
            `[yaoyao-memory:persist] indexTurn failed: ${e2 instanceof Error ? e2.message : String(e2)}`,
          );
        }
      }
      if (rows.length > 0 && embedding) {
        try {
          ...
          api.logger.debug?.(
            `[yaoyao-memory:persist] Batch vector store failed: ${e2 instanceof Error ? e2.message : String(e2)}`,
          );
        }
      }
    },
