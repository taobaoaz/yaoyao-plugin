...
export function registerCaptureHook(
  ...
  const captureDebouncer: CaptureDebouncer = createCaptureDebouncer(
    ...
    createFlushHandler(persist, writeQueue, clawBridge, forwardCapture, api),
  );
  ...
  api.on("agent_end", async (event: unknown, ctx: unknown) => {
    ...
    const result = await processCaptureEvent(
      event, ctx, config, api, store, verifyActive, scopeManager, agentId,
      llmClient, audit, dedupEngine, db, embedding, skipLocalIndexing,
    );
