...
export async function processCaptureEvent(
  ...
  try {
    ...
    if (watermark.level !== 'none') {
      api.logger.info?.(
        `[yaoyao-memory:capture] Watermark ${watermark.level} (${(watermark.ratio * 100).toFixed(1)}%)`,
      );
    ...
    // Anti-hallucination
    const { riskTag, specCheck, corrCheck } = runAntiHallucination(
      cctx.userContent,
      cctx.indexableAsst,
      verifyActive,
    );
    handleMermaidOffload(
      ...
      sessionKey,
      cctx.userContent + '\n' + cctx.asstContent,
      capCfg.enableOffload,
      capCfg.offloadThreshold,
    );

    // Meta object
    const { meta } = await buildMetaObj(
      cctx.userContent,
      cctx.indexableAsst,
      scopeManager,
      agentId,
      specCheck,
      corrCheck,
      skipLocalIndexing ? false : capCfg.enableL1,
      watermark.skipL1 || false,
      capCfg.brainMode,
      llmClient,
      api.logger,
      capCfg.maxMemoriesPerSession,
      config,
    );
    ...
    if (dedupResult.isDuplicate) {
      api.logger.debug?.(
        `[yaoyao-memory:capture] Duplicate (stage=${dedupResult.stage}, conf=${dedupResult.confidence.toFixed(3)}): ${dedupResult.reason}`,
      );
      return skipResult(
        sessionKey,
        date,
