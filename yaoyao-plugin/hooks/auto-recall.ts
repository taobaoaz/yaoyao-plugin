...
export function registerRecallHook(
  ...
  api.logger.info(
    `[yaoyao-memory] Registering before_prompt_build hook (auto-recall${embedding ? ' + vector' : ''})${clawBridge ? ' [coexist: claw-core recall primary]' : ''}`,
  );
  ...
  const resultCache = new SimpleLRU<string, import('../storage/bridge.ts').SearchResult[]>({
  ...
  const handler = async (event: unknown, ctx: unknown) => {
    const recallAsync = async () => {
      try {
        ...
        api.logger.error?.(
          `[yaoyao-memory:recall] Hook error: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    };
    ...
    try {
      await Promise.race([
        ...
        new Promise<void>((_, reject) =>
          setTimeout(() => reject(new Error('recall timeout')), baseCfg.timeoutMs),
        ),
      ]);
    } catch (timeoutErr) {
