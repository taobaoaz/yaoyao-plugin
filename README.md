...
仅当 `celiaBridge.enabled=true` 且检测到 `memory-celia` 占槽时注册。这些工具在空环境本不存在，是把官方 celia 独有能力**代理暴露**为 yaoyao 工具（标注 `[via celia]`，数据来自 celia）：
...
同时，主表中与 celia 重叠的工具（`memory_save` / `memory_search*` / `memory_forget` / `memory_list`）在委托模式（`mode: "delegate"`）下会**转发给 celia 执行**（单一数据源），失败自动 fallback 回 yaoyao 自实现。
...
**当前状态：799 测试全部通过**，覆盖 194 个测试模块，覆盖率重点模块包括：
...
检测到 `memory-celia` 占槽时，yaoyao **自动关闭**会与之冲突的 3 个生命周期 hook，保留全部 40 个工具作为按需增强层：

| 组件 | 空环境 | memory-celia 占槽 |
|---|---|---|
| auto-capture（L0 写入） | ✅ | ❌ 关闭（celia 负责 L0） |
| auto-recall（prompt 注入） | ✅ | ❌ 关闭（避免双注入） |
| heartbeat-recall | ✅ | ❌ 关闭 |
...
```jsonc
{
  "celiaBridge": {
    "enabled": true,
    "mode": "delegate",            // delegate=委托+暴露 celia 工具; read-only=只读库增强
    "serverBinaryPath": "",        // 留空自动探测 ~/.openclaw/extensions/celia_memory/current/bin/
    "dbPath": "~/.openclaw/workspace/memory/celia_memory/celia_memory.db"
  }
}
...
**安全保证**：委托调用失败会自动 fallback 回 yaoyao 自实现，celia 故障不会中断 yaoyao；yaoyao 绝不直接写入 celia 的数据库。
...
- [x] celia 委托桥（可选）：重叠工具委托 celia 执行 + 暴露 dream/scene/global 等 celia 独有工具
- [x] celia 只读桥（read-only 模式）：读官方库做增强分析
- [x] 新增 `celiaBridge` 配置项（默认关闭，空环境零影响）
- [x] 测试 768 → **799**（+16：celia 映射 12 + 共存检测 4）
