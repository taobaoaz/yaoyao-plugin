...
| 39 | `memory_setup` | **v1.9.1+** 配置自检与首次引导（检测运行模式/共存桥/向量/数据量，返回优化建议 + 安装向导路径） |

> 主表始终交付 39 个工具（空环境与 memory-celia 占槽环境一致）。共存模式下另可激活下表的 celia proxy 工具。
...
**当前状态：826 测试全部通过**，覆盖 204 个测试模块，覆盖率重点模块包括：
...
```jsonc
{
  "celiaBridge": {
    "enabled": true,
    "mode": "delegate",            // delegate | read-only（"readonly" 为别名，等价；见下表）
...
- [x] mode 容错：`read-only` / `readonly` / `read_only` 自动归一化（照任意配置文档填写都正确）
- [x] **首次对话自动引导**：agent 自主发现配置状态（共存桥/向量/数据空），首次对话注入提示 + `memory_setup` 工具随时复查
