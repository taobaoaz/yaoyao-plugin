...
export class SFTPAdapter {
    ...
    esc(p) { return p.replace(/"/g, '\\"'); }
    async upload(localPath, remotePath) {
        try {
            const { ok } = await this.runSftp([`put "${this.esc(localPath.replace(/\\/g, "/"))}" "${this.esc(remotePath)}"`]);
            return ok;
        }
        catch {
            return false;
    ...
    async download(remotePath, localPath) {
        try {
            ...
            const { ok } = await this.runSftp([`get "${this.esc(remotePath)}" "${this.esc(localPath.replace(/\\/g, "/"))}"`]);
            return ok;
        }
        catch {
            return false;
