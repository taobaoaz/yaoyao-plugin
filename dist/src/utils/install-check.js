...
export function runInstallCheck() {
    ...
    if (satisfiesVersion(nodeVersion, versions.nodeRange)) {
        ...
        warnings.push(`Node.js ${nodeVersion} 低于推荐版本 ${versions.nodeRange}。` +
    ...
    if (backend === "node-sqlite") {
        ...
        warnings.push("无 SQLite 可用。插件将运行在纯文件降级模式（file-db）。" +
    ...
    if (backend === "node-sqlite") {
        try {
            ...
            warnings.push("sqlite-vec 未安装。向量搜索不可用，FTS5 纯文本搜索仍正常工作。" +
    ...
    try {
        ...
        warnings.push(`临时目录不可写: ${e.message}。备份、导出功能可能受影响。`);
    }
    // 6. Git
    try {
...
export function formatInstallCheck(result) {
    const lines = [
        `## 🔧 环境能力报告`,
        ``,
        `**运行模式**: ✅ 可以运行`,
