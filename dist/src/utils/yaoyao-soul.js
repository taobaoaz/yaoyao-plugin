...
function buildEmotionTrie() {
    ...
    for (const [emotion, words] of Object.entries(CN_LEXICON)) {
...
function buildENSet() {
    const result = {};
    for (const [emotion, words] of Object.entries(EN_LEXICON)) {
...
export function detectSentiment(text) {
    ...
    for (const w of words) {
        for (const emotion of Object.keys(enSet)) {
...
export class YaoyaoSoul {
    ...
    totalFailure = 0;
    messageLengths = [];
    lastInterval = 0; // ms since last interaction
    // L3 persona hints
    ...
    update(opts = {}) {
        ...
        if (this.messageLengths.length > MOOD_CONFIG.maxMsgLenHistory)
        ...
        if (this.lastScores.length > MOOD_CONFIG.maxTrendHistory)
    ...
    getGuidance() {
        ...
        const verbosity = state.energy === "low" ? "concise"
    ...
    computeMood(text) {
        ...
        const label = score > MOOD_CONFIG.positiveThreshold ? "positive"
    ...
    computeIntensity() {
        if (this.lastInterval <= 0)
            return ENERGY_CONFIG.defaultIntensity;
    ...
    computeEnergy(intensity, msgLen) {
        ...
        if (isShort && intensity > ENERGY_CONFIG.highIntensityThreshold)
            return "low";
        if (isShort)
            return "high";
        // Medium-length messages: use intensity
        if (intensity > ENERGY_CONFIG.highIntensityThreshold)
            return "low";
        if (intensity < ENERGY_CONFIG.lowIntensityThreshold)
            return "high";
        return "medium";
    }
    ...
    computeTrust(success, fail) {
        if (success > 0 || fail > 0) {
            ...
            const cappedRate = this.totalSuccess + this.totalFailure < TRUST_CONFIG.minObservations
                ...
                : smoothed;
            if (cappedRate > TRUST_CONFIG.highThreshold)
                return "high";
            if (cappedRate < TRUST_CONFIG.lowThreshold)
                return "low";
            return "medium";
        }
    ...
    detectTrend() {
        ...
        if (delta > MOOD_CONFIG.trendRisingThreshold)
            return "rising";
        if (delta < MOOD_CONFIG.trendFallingThreshold)
            return "falling";
        return "stable";
    }
    ...
    applyConfidenceDecay(state) {
        ...
        if (idleMs < DECAY_CONFIG.idleGraceMs)
            return state;
