...
export function runMemoryStatsCheck(baseDir, memDir, createCompatDB) {
    try {
        ...
        const total = statsDb.prepare("SELECT COUNT(*) as c FROM yaoyao_meta").get();
        const tierDist = statsDb.prepare("SELECT tier, COUNT(*) as c FROM yaoyao_meta GROUP BY tier").all();
        const avgAge = statsDb.prepare("SELECT AVG(julianday('now') - julianday(created_at)) as d FROM yaoyao_meta").get();
        statsDb.close();
        return {
            name: "记忆统计",
            status: "pass",
