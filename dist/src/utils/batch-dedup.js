 ...
 */
export function isDuplicateOfRecent(candidate, recentMemories, threshold = 0.85) {
...
export function batchDedup(texts, threshold = 0.85) {
    if (texts.length === 0) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        if (candidates[i].isBatchDuplicate)
            continue;
        for (let j = i + 1; j < candidates.length; j++) {
            if (candidates[j].isBatchDuplicate)
                continue;
