...
export function createRetainTool(store, db) {
  return {
    ...
    label: "Memory Retain",
    description:
      "🧠 记忆增强/反遗忘机制 — 检测重要但长期未被召回的记忆，生成强化建议。通过追踪召回历史和重要标签，防止关键记忆被遗忘。支持 check=检查风险, boost=强化, important=标记重要。",
    parameters: {
      type: "object",
      properties: {
        action: {
    ...
    execute: withErrorHandling(async (_id, params) => {
      const actionAliases = { "检查": "check", "强化": "boost", "重要": "important" };
...
function ensurePipelineDir(baseDir) {
  ...
  if (!fsMod.existsSync(d)) {
    fsMod.mkdirSync(d, { recursive: true });
  }
}
...
async function handleBoost(store, db, keyword, filename, reason) {
  ...
  try {
    ...
    fsMod.appendFileSync(boostFile, JSON.stringify(record) + "\n", "utf-8");
  } catch (err) {
...
async function handleImportant(store, keyword, filename, reason) {
  ...
  try {
    ...
    fsMod.writeFileSync(importantFile, JSON.stringify(tags, null, 2), "utf-8");
  } catch (err) {
...
function loadBoostRecords(baseDir) {
  ...
  const records = [];
  try {
    if (!fsMod.existsSync(fp)) return records;
    const raw = fsMod.readFileSync(fp, "utf-8");
