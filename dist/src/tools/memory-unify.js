 ...
 * Provides a single interface to query all memory backends:
 * - OpenClaw built-in memory (main.sqlite)
 * - .dreams (short-term recall + events)
 * - yaoyao's own FTS5 + vec index
 */
...
async function handleSearchAll(store, query) {
  ...
  let ocResults = null;
  try {
    ocResults = queryOpenClawDB("SELECT path, text FROM chunks_fts WHERE text MATCH ? LIMIT 5", [query]);
  } catch {
    // FTS5 might fail on special chars, try LIKE
    ocResults = queryOpenClawDB("SELECT path, text FROM chunks WHERE text LIKE ? LIMIT 5", ["%" + query + "%"]);
  }
  lines.push("## OpenClaw 内置记忆");
  if (ocResults && ocResults.length > 0) {
    ocResults.forEach(r => lines.push("- **" + r.path + "**: " + r.text.substring(0, 100) + "..."));
  } else {
    lines.push("- 无匹配");
  }
  ...
  const dreams = readDreams(store.baseDir);
  const dreamMatches = dreams.events.filter(e => JSON.stringify(e).toLowerCase().includes(query.toLowerCase()));
  if (dreamMatches.length > 0) {
    dreamMatches.slice(0, 5).forEach(e => lines.push("- " + JSON.stringify(e).substring(0, 120)));
  } else {
    lines.push("- 无匹配");
  }
