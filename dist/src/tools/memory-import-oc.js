...
export function createImportOCTool(store, db) {
    return {
        ...
        execute: withErrorHandling(async (_id, params) => {
            ...
            const lastImportedId = Number(db.getConfig("oc_import_last_id", "0"));
            ...
            for (const chunk of newChunks) {
                try {
                    ...
                    const date = dateMatch ? dateMatch[1] : (chunk.updated_at ? chunk.updated_at.slice(0, 10) : now);
                    ...
                    const rowId = db.indexTurn(`${sourceTag} ${text.slice(0, 1900)}`, "", date, "oc-import");
                    if (rowId > 0) {
                        imported++;
