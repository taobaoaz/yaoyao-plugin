...
function isStopWord(word) {
  ...
  if (ENGLISH_STOP.has(lower)) return true;
  if (CHINESE_STOP.has(word)) return true;
  return false;
}
...
function extractTokens(text) {
  ...
  for (const w of englishWords) {
    ...
    if (!isStopWord(lower)) tokens.push(lower);
  }
  ...
  return {
    ...
    execute: withErrorHandling(async (_id, params) => {
      ...
      if (allTokens.length === 0) {
        return { content: [{ type: "text", text: `在 ${period === "all" ? "全部" : `近 ${period}`} 周期内没有提取到足够的关键词。` }] };
      }
      ...
      const trends = sorted.map(([word, count]) => {
        ...
        let emoji, direction;
        if (early === 0 && late > 0) { emoji = "🆕"; direction = "新增话题"; }
        else if (late === 0 && early > 0) { emoji = "📉"; direction = "已消失"; }
        else if (late > early * 1.5) { emoji = "📈"; direction = "快速上升"; }
        else if (late > early * 1.2) { emoji = "↗️"; direction = "缓慢上升"; }
        else if (early > late * 1.5) { emoji = "📉"; direction = "快速下降"; }
        else if (early > late * 1.2) { emoji = "↘️"; direction = "缓慢下降"; }
        else { emoji = "➡️"; direction = "基本稳定"; }
      ...
      lines.push("", "## 📋 趋势摘要", "");
      if (rising.length > 0) lines.push(`**🔥 上升话题**: ${rising.map(t => `${t.word}（${t.count}）`).join("、")}`);
      if (falling.length > 0) lines.push(`**🧊 下降话题**: ${falling.map(t => `${t.word}（${t.count}）`).join("、")}`);
      if (stable.length > 0) lines.push(`**➡️ 稳定话题**: ${stable.map(t => `${t.word}（${t.count}）`).join("、")}`);
