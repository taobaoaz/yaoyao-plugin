...
function loadTags(db, dbPath) {
    const tags = new Map();
    try {
        const { DatabaseSync } = _require("node:sqlite");
        const tagDb = new DatabaseSync(dbPath, { allowExtension: true });
        try {
            const rows = tagDb.prepare("SELECT tag, memory_id FROM memory_tags").all();
...
// ── Build Graph ──
function buildGraph(query, db, dbPath, memoryDir, depth, scenes, tags, embedding, queryEmbedding) {
    const nodes = new Map();
    const edges = new Map();
