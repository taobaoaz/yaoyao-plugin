...
import { MultimodalStorage, newMultimodalId, sha256OfBytes } from "./storage.js";
function guessMime(t, hint) {
    if (hint)
        return hint;
    if (t === "image")
        return "image/png";
...
function guessExt(mimeType) {
    ...
    if (m.includes("quicktime"))
        return "mov";
    if (m.includes("matroska"))
        return "mkv";
    return "bin";
}
function describeModality(t) {
...
export class MultimodalProcessor {
        ...
        try {
            ...
            if (input.sourceType === "base64") {
                ...
                try {
                    ...
                    return { ok: false, error: "cannot access path: " + (e.message || String(e)) };
                }
    ...
    formatEntry(e, snippet) {
        ...
        if (snippet)
            lines.push("  片段: " + snippet);
        if (e.extractedText)
            lines.push("  提取: " + e.extractedText.slice(0, 120));
        if (e.linkedMemoryIds.length)
            lines.push("  关联文本记忆: " + e.linkedMemoryIds.join(", "));
        return lines.join("\n");
    }
}
