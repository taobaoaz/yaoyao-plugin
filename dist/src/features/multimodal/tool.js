...
import { MultimodalProcessor } from "./processor.js";
const VALID_ACTIONS = ["save", "get", "list", "search", "link", "delete"];
export function createMultimodalTool(cfg) {
    ...
    return {
        ...
        description: "多模态记忆（hidden feature，默认关闭）。支持 image / audio / video 三种模态。" +
            "动作: save / get / list / search / link / delete。sourceType 支持 url / path / base64。" +
            "注意: 此工具仅在 config.multimodal.enabled=true 时注册。",
        parameters: {
            type: "object",
            properties: {
                action: { type: "string", enum: VALID_ACTIONS, default: "list" },
                id: { type: "string" },
        ...
        execute: withErrorHandling(async (_id, params) => {
            ...
            if (action === "get") {
                ...
                if (!id)
                    return { content: [{ type: "text", text: "❌ id 必填" }] };
                const e = processor.get(id);
                if (!e)
                    return { content: [{ type: "text", text: "❌ 未找到: " + id }] };
            ...
            if (action === "list") {
                ...
                if (r.items.length === 0)
                    return { content: [{ type: "text", text: "📭 没有匹配的多模态记忆（共 0 条）" }] };
                const lines = ["📚 多模态记忆列表 (共 " + r.total + " 条，显示 " + r.items.length + " 条):"];
                for (const e of r.items)
                    lines.push(processor.formatEntry(e));
                return { content: [{ type: "text", text: lines.join("\n") }] };
            }
            if (action === "search") {
                const query = String(params.description || params.query || "");
                if (!query)
                    return { content: [{ type: "text", text: "❌ description (查询) 必填" }] };
                const type = params.type ? String(params.type) : undefined;
                const limit = params.limit ? Number(params.limit) : 10;
                const r = processor.search(query, { type, limit });
                if (r.length === 0)
                    return { content: [{ type: "text", text: "🔍 没有匹配 \"\" + query + \"\" 的多模态记忆" }] };
                const lines = ["🔍 多模态搜索结果 (\"\" + query + \"\", " + r.length + " 条):"];
                for (const e of r)
                    lines.push(processor.formatEntry(e, e.snippet));
                return { content: [{ type: "text", text: lines.join("\n") }] };
            }
            if (action === "link") {
                const id = String(params.id || "");
                ...
                if (!id || !memoryId)
                    return { content: [{ type: "text", text: "❌ id 和 memoryId 都必填" }] };
