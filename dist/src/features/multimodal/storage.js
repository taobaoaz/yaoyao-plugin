...
export class MultimodalStorage {
    ...
    loadIndex() {
        return readJson(join(this.root, INDEX_FILE), []);
    }
    saveIndex(entries) {
        atomicWriteJson(join(this.root, INDEX_FILE), entries);
    }
    saveContent(id, ext, bytes) {
    ...
    remove(id, ext) {
        ...
        this.saveIndex(next);
        try {
            unlinkSync(join(this.root, META_SUBDIR, id + ".json"));
        }
        catch { /* ignore */ }
        if (ext) {
            try {
                unlinkSync(join(this.root, CONTENT_SUBDIR, id + "." + safeExt(ext)));
            }
            catch { /* ignore */ }
        }
        return true;
