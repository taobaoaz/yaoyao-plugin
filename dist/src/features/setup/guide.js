...
export function renderSetupPrompt(state) {
    if (state.ready)
        return "";
    const lines = [];
...
export function renderSetupReport(state) {
    const lines = [];
    ...
    const order = ["warn", "tip", "info"];
    const sorted = [...state.findings].sort((a, b) => order.indexOf(a.severity) - order.indexOf(b.severity));
