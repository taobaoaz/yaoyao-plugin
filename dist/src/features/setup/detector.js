...
export function detectSetup(input) {
    const findings = [];
    const mode = input.coexistMode === "coexist" ? "coexist" : "standalone";
    ...
    if (mode === "coexist") {
        ...
        if (!bridgeOn) {
            findings.push({
                ...
                detail: `yaoyao 已自动进入共存模式（捕获/召回已禁用以避免与官方系统冲突）。` +
                    `当前 40 个工具使用 yaoyao 独立存储。如需统一数据源或使用 dream/scene 等官方能力，` +
                    `可开启 celiaBridge。`,
                action: `在 openclaw.json 的 yaoyao-memory.config 中添加：\n` +
    ...
    if (input.memoryCount === 0) {
        findings.push({
            ...
            severity: "tip",
            title: "记忆库为空",
            detail: `还没有任何记忆数据。yaoyao 会在对话中自动捕获，但首次使用时可能感觉"没有记忆"。` +
                `standalone 模式下对话结束后会自动落盘；coexist 模式下捕获已关闭，需手动或通过官方系统积累。`,
            action: `可手动测试：memory_save({content:"测试记忆"})，再用 memory_search 查询。` +
    ...
    if (!input.embeddingEnabled && input.cap.backend !== "file-db") {
        findings.push({
            ...
            severity: "tip",
            title: "向量搜索未启用",
            detail: `当前使用 FTS5 纯文本搜索。开启向量搜索可显著提升语义召回质量（如"咖啡"能召回"拿铁"）。`,
            action: `配置 embedding.enabled=true + apiKey。详见 docs/install-guide.md §五·2。`,
        });
    }
