...
export function createMultiSignalSearchTool(db, embedding) {
    return {
        ...
        description: "Searches using multi-signal fusion (mem0 v3 style). Supports rrf (default) and additive modes. " +
        ...
        execute: withErrorHandling(async (_id, params) => {
            ...
            if (fusionMode === "additive") {
                ...
                const signalConfig = { temporalHalfLifeDays: temporalDecayDays };
                if (!enableEntityBoost)
                    signalConfig.entityBoostMax = 0;
                const fused = multiSignalFusion(query, ftsResults, vecResults.map((r) => ({ ...r, asst_text: "", timestamp: undefined, importance: undefined, scope: undefined })), allResults, signalConfig);
