...
export class EpisodicCache {
     ...
     */
    query(userQuery, sessionKey, maxResults = 3) {
        if (this.buffer.length === 0)
            return [];
        const queryTokens = new Set(userQuery.toLowerCase().split(/[\s\p{P}]+/u).filter(t => t.length >= 2));
...
export function getGlobalEpisodicCache() {
    if (!globalCache) {
