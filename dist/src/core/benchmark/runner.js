 ...
 */
import { getBenchmarkSuite } from "./cases.js";
function evaluateAnswer(response, expected) {
...
export async function runBenchmark(options) {
    ...
    for (const testCase of cases) {
        ...
        if (options?.searchFn) {
            ...
            retrievedMemories = testCase.conversation.filter((line) => testCase.expectedAnswer.split(/\s+/).some((word) => line.toLowerCase().includes(word.toLowerCase())));
        }
        // Simulate answering
        let response = "";
        if (options?.answerFn) {
    ...
    for (const testCase of cases) {
        ...
        if (!byCategory[cat])
            byCategory[cat] = { passed: 0, total: 0, avgScore: 0 };
        byCategory[cat].total++;
        if (result.passed)
            byCategory[cat].passed++;
        byCategory[cat].avgScore += result.score;
        if (!byDifficulty[diff])
            byDifficulty[diff] = { passed: 0, total: 0, avgScore: 0 };
