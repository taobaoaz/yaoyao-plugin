...
export function ensureTagTable(db) {
    ...
    db.exec("CREATE TABLE IF NOT EXISTS yaoyao_tags (" +
...
export function getPopularTags(db, limit) {
    ...
    if (!Number.isFinite(limit) || limit < 1)
        limit = 20;
    const stmt = db.prepare("SELECT tag, COUNT(*) as count FROM yaoyao_tags GROUP BY tag ORDER BY count DESC LIMIT ?");
...
export function getTotalTagCount(db) {
    ...
    const row = db.prepare("SELECT COUNT(*) as c FROM yaoyao_tags").get();
...
export function cleanOrphanTags(db) {
    ...
    const result = db.prepare("DELETE FROM yaoyao_tags WHERE memory_id NOT IN (SELECT rowid FROM yaoyao_meta)").run();
    return Number(result.changes ?? 0);
}
