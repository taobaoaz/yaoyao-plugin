...
export default definePluginEntry({
 ...
 // 当运行 dist/index.js 时，import.meta.url 指向 dist/ 目录
 // 因此自检路径要用 "../dist/xxx"（从 dist/ 回到插件根目录再进 dist/）
 const _selfUrl2 = import.meta.url;
 ...
 const resolved = new URL(relPath, _selfUrl2);
 ...
 const migrated = [];
 // Step 1: collect all .json files recursively
 const jsonFiles = [];
 const walk = (d) => {
 ...
 // Step 3: delete entire old skill directory (all configs are now migrated)
 fs.rmSync(dir, { recursive: true });
 if (migrated.length > 0) {
 api.logger.info(`[yaoyao-memory] 已迁移 ${migrated.length} 个配置文件并从 ${name} 转移（${migratedDir}），旧目录已清理`);
 } else {
