...
import { resolveWeights, normalizeWeights } from "./weights.ts";
import { getRelatedMemories } from "../graph/mutators.ts";
import { queryFacts } from "../atomic/query.ts";
...
export function adaptiveSearch(
  query: string,
  options?: {
    semanticSearch?: (q: string) => Array<{ id: string; score: number }>;
    keywordSearch?: (q: string) => Array<{ id: string; score: number }>;
    maxResults?: number;
  },
): SearchResult[] {
