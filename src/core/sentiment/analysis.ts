/**
 * core/sentiment/analysis.ts — Sentiment analysis algorithm.
 *
 * Pure detection functions: detectSentiment and summarizeMood.
 */
import type { SentimentResult, EmotionLabel } from "./types.ts";
import { cn, en, JOY_MARKERS, SAD_MARKERS, ANGRY_MARKERS, SURPRISE_MARKERS, NEGATION_PREFIXES } from "./lexicon.ts";
...
export function detectSentiment(text: string): SentimentResult {
  ...
  for (let i = 0; i < text.length - 1; i++) {
    ...
    for (const emotion of Object.keys(cn) as EmotionLabel[]) {
      ...
      if (score > 0) {
        if (isNegated && emotion === "joy") emotionScores.sadness += score;
        else if (isNegated) emotionScores[emotion] += score * 0.5;
        else emotionScores[emotion] += score;
      }
    }
  }
  ...
  let emoji: string;
  if (diff > 0.15) { label = "positive"; emoji = positive > 0.8 ? "🥰" : positive > 0.6 ? "😊" : "🙂"; }
  else if (diff < -0.15) { label = "negative"; emoji = negative > 0.8 ? "😢" : negative > 0.6 ? "😟" : "😕"; }
...
export function summarizeMood(texts: string[]): string {
  ...
  for (const r of results) {
    for (const e of r.topEmotions) emotionCounts[e] = (emotionCounts[e] || 0) + 1;
  }
  ...
  let summary = "";
  if (posRatio > 0.6) summary = "😊 整体心情不错";
  else if (negRatio > 0.6) summary = "😢 最近似乎有些烦恼";
  else if (posRatio > negRatio) summary = "🙂 总体偏积极";
  else if (negRatio > posRatio) summary = "😟 最近有点低落";
  else summary = "😐 情绪平稳";

  if (topEmotion.length > 0) summary += ` | 主要情绪: ${topEmotion.map(([e, c]) => `${e}(${c}次)`).join(", ")}`;
  return summary;
}
