/**
 * core/verify/verify.ts — anti-hallucination verification engine.
 *
 * Provides fact-checking against stored memories without LLM calls.
 * Uses FTS5 search + keyword overlap scoring.
 */
...
export function detectSpeculative(text: string): {
  ...
  // Count density: more markers = higher speculation
...
export function scoreEvidence(
  ...
  // Extract keywords from claim (remove stopwords, keep nouns/verbs)
  ...
  let verdict: "confirmed" | "partial" | "unconfirmed" | "contradicted" = "unconfirmed";
  let confidence = 0;
  let reasoning = "";

  // Contradiction detection FIRST: if claim and top memory have negation mismatch
...
/** Extract meaningful keywords from text — character-based for Chinese, word-based for English */
function extractKeywords(text: string): string[] {
  ...
  // For Chinese: extract individual characters + 2-grams
  ...
  // For English/alphanumeric: word-based extraction
  const words = lower
    .replace(/[^a-z0-9\s]/g, " ")
    .split(/\s+/)
...
function hybridOverlap(claim: string, snippet: string): number {
  ...
  // If no English words, use only Chinese char score
  if (claimWords.length === 0) return charScore;
  // If no Chinese chars, use only English word score
  if (claimChars.size === 0) return wordScore;
  // Otherwise average (Chinese chars weighted slightly more for mixed content)
