...
export function syncMarkdownToFTS(
  ...
  try {
    ...
    try {
      ...
      dbReadFailed = true;
      logger?.warn?.(
        `[yaoyao-memory:md-sync] Could not read DB row count: ${err instanceof Error ? err.message : String(err)}`
      );
    }

    if (dbReadFailed) {
      logger?.warn?.(
        "[yaoyao-memory:md-sync] DB read failed — aborting sync to avoid silent bulk-import. " +
