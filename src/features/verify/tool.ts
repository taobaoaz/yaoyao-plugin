/**
 * features/verify/tool.ts — memory_verify tool (anti-hallucination fact-checking).
 *
 * Verifies a claimed fact against stored memories.
 * Returns: confirmed / partial / unconfirmed / contradicted
 */
...
export function createVerifyTool(db: DBBridge): ToolRegistration {
  return {
    ...
    execute: withErrorHandling(async (_id: string, params: Record<string, unknown>) => {
      ...
      // Step 1: Detect if the claim itself is speculative
      const specCheck = detectSpeculative(claim);

      // Step 2: Search memory for evidence
      let results: Array<{ snippet: string; filename: string }> = [];
      try {
