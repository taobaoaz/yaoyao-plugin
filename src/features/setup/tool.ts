...
import { collectSetupInput, safeMemoryCount } from "./collector.ts";
import { detectSetup } from "./detector.ts";
import { renderSetupReport } from "./guide.ts";
...
export function createSetupTool(deps: SetupToolDeps): ToolRegistration {
  return {
    id: "memory_setup",
    name: "memory_setup",
    label: "配置自检与引导",
    description:
      "🚀 检查 yaoyao-memory 的当前配置状态（运行模式、共存桥、向量搜索、数据量），" +
