...
export type BackpropRelation = "supersedes" | "reinforces" | "elaborates" | "contradicts" | "none";

export interface BackpropResult {
  /** Relationship type detected */
  relation: BackpropRelation;
  /** ID of the affected historical memory */
  targetId: number;
  /** Confidence in this relationship (0–1) */
  confidence: number;
  /** What action was taken on the target memory */
  action: "skip" | "tag_updated" | "confidence_bumped" | "marked_superseded" | "marked_contradicted" | "none";
  /** Diagnostic reason */
  reason: string;
}
...
export interface BackpropConfig {
  enabled: boolean;
  /** Minimum text similarity to consider a relationship (0–1) */
  minSimilarity: number;
  /** How many recent memories to scan */
  scanCount: number;
  /** Bump confidence by this amount for REINFORCES */
  reinforceBump: number;
  /** Maximum confidence cap */
  maxConfidence: number;
  /** MemRL Phase-1 pre-filter threshold (trigram Jaccard). Lower = more aggressive filter. */
  preFilterThreshold: number;
}

const DEFAULT_CONFIG: BackpropConfig = {
  enabled: true,
  minSimilarity: 0.4,
  scanCount: 20,
  reinforceBump: 0.05,
...
function trigramJaccardQuick(a: string, b: string): number {
...
function detectContradiction(textA: string, textB: string): boolean {
  ...
  if (contradictionWords.test(a) && entityOverlap > 0.3) return true;
  if (contradictionWords.test(b) && entityOverlap > 0.3) return true;
...
): { relation: BackpropRelation; confidence: number } {
...
export class MemoryBackprop {
  ...
  constructor(config?: Partial<BackpropConfig>) {
  ...
  async process(
    ...
    let rawCandidates: Array<{ id: number; text: string; meta?: string }> = [];
    ...
    const preQualified: Array<{ id: number; text: string; meta?: string }> = [];
    for (const c of rawCandidates) {
      if (!c.text || c.text === newText) continue;
    ...
    for (const candidate of preQualified) {
      ...
      let action: BackpropResult["action"] = "none";
      let reason = "";
  ...
  private _fingerprint(text: string): string {
    let hash = 0x811c9dc5;
