...
export function runHealthcheck(baseDir?: string): HealthResult {
  ...
  let gatewayVer = (openclawSdk as unknown as Record<string, unknown>).OPENCLAW_VERSION as string
  ...
  if (dbCap.nodeSqliteAvailable || dbCap.betterSqlite3Available) {
    ...
    try {
      ...
      const mode = testDb.prepare("PRAGMA journal_mode").get() as Record<string, unknown> | undefined;
      testDb.close();
