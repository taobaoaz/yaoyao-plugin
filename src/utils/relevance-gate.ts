...
export interface RelevanceGateConfig {
  enabled: boolean;
  /** Minimum composite score to pass (0–1) */
  minScore: number;
  /** Novelty: text similarity threshold below which considered "new" */
  noveltyThreshold: number;
  /** Time decay: half-life in seconds for rapid repeats */
  ttlHalfLifeSec: number;
  /** Info density: minimum distinct tokens per 100 chars */
  minInfoDensity: number;
  /** How many recent memories to check for novelty */
  noveltyLookback: number;
  /** Penalty multiplier when content is entirely in recent history */
  redundancyPenalty: number;
  /** Repeat count within half-life to block outright */
  repeatBlockThreshold: number;
  /** Minimum content length (chars) to consider meaningful */
  minContentLength: number;
  /** SimpleMem-style entropy filter threshold (lower = more aggressive filtering) */
  entropyThreshold: number;
}

const DEFAULT_CONFIG: RelevanceGateConfig = {
  enabled: true,
  minScore: 0.45,
...
function estimateInfoDensity(text: string): number {
  ...
  for (const t of tokens) {
    if (t.length > 3 && !STOP_WORDS.has(t)) entityTokens++;
  }
...
export class RelevanceGate {
  private config: RelevanceGateConfig;
  private decayRing: TemporalDecayRing;

  constructor(config?: Partial<RelevanceGateConfig>) {
  ...
  async evaluate(
    ...
    if (recentTexts.length > 0) {
      ...
      for (const recent of recentTexts.slice(0, this.config.noveltyLookback)) {
        const recentTokens = new Set(
          recent.toLowerCase().split(/[\s,.;:!?]+/).filter((t) => t.length > 2),
        );
        if (tokens.size === 0 || recentTokens.size === 0) continue;
    ...
    else if (text.length < 30) confidence = 0.5;
    else if (text.length > 10000) confidence = 0.6;
