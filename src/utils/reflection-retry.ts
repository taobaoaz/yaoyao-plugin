/**
 * Reflection Retry — Transient error classifier & retry helper (from Brain v1.1.0)
 * Zero external dependency. Detects retryable vs non-retryable errors.
 */
...
const NON_RETRY_PATTERNS: RegExp[] = [
  /\b401\b/i,
  /\bunauthorized\b/i,
  /invalid api key/i,
  /invalid[_ -]?token/i,
...
/** Check if an error is a transient upstream failure (network, timeout, 502/503/504). */
...
/** Check if an error should NEVER be retried (auth, quota, policy, invalid input). */
export function isNonRetryError(error: unknown): boolean {
  const msg = toErrorMessage(error);
  return NON_RETRY_PATTERNS.some((pattern) => pattern.test(msg));
}

/** Classify whether an error is retryable. */
export function classifyRetry(input: RetryClassifierInput): RetryClassifierResult {
