...
/** One mapping entry: yaoyao tool → celia tool + arg transform. */
export interface CeliaToolMapping {
  /** The celia tool name to invoke. */
  celiaTool: string;
  /** yaoyao params → celia arguments. */
  map: (yaoyaoParams: Record<string, unknown>) => Record<string, unknown>;
  /** Optional result normalization (defaults to passthrough). */
  unmap?: (celiaText: string) => string;
}

/** Shared sessionId extractor: yaoyao tools don't carry it, so synthesize one. */
function synthesizeSessionId(p: Record<string, unknown>): string {
  return (p.sessionId as string) || (p.session_id as string) || `yaoyao-${Date.now()}`;
}

/** yaoyao tools spell "limit" variously; pick whichever is present. */
function pickLimit(p: Record<string, unknown>, dflt: number): number {
...
function pickQuery(p: Record<string, unknown>): string {
  return (p.query as string) ?? (p.keyword as string) ?? (p.q as string) ?? "";
}
 ...
 */
export const CELIA_DELEGATION_MAP: Record<string, CeliaToolMapping> = {
  // memory_save({content}) → celia memory_store({text})
  // yaoyao uses `content`; celia uses `text`.
  memory_save: {
    celiaTool: "memory_store",
    map: (p) => ({ text: (p.content as string) ?? (p.text as string) ?? "" }),
  },
...
export function isDelegatable(yaoyaoToolName: string): boolean {
  return yaoyaoToolName in CELIA_DELEGATION_MAP;
}

/** Get the mapping for a tool, or null. */
export function getMapping(yaoyaoToolName: string): CeliaToolMapping | null {
