...
function parseMetadata(r: SearchResult): Record<string, unknown> {
 ...
 */
export function applyFourSignalFusion(
  results: SearchResult[],
  weights?: { semantic?: number; temporal?: number; confidence?: number; relational?: number },
): SearchResult[] {
  const w = {
  ...
  return results.map((r) => {
    ...
    const expiryAt = meta.expiryAt as string | undefined;
    if (expiryAt) {
    ...
    if (meta.speculative === true) {
      ...
      const specLevel = meta.confidence as string | undefined;
    ...
    const memType = meta.memoryType as string | undefined;
    if (memType === "preference" || memType === "entity") relationalScore = Math.min(1.0, relationalScore + 0.15);
    if (memType === "goal") relationalScore = Math.min(1.0, relationalScore + 0.1);
