...
describe("buildMergedEntry", () => {
  it("deduplicates lines across members", () => {
    const merged = buildMergedEntry([
      { id: "1", text: "Line A\nLine B", category: "fact", importance: 0.5, timestamp: 1, scope: "user" },
      { id: "2", text: "Line B\nLine C", category: "fact", importance: 0.7, timestamp: 2, scope: "user" },
    ]);
...
describe("buildTextClusters", () => {
  it("returns empty when entries < minClusterSize", () => {
    const clusters = buildTextClusters(
      [{ id: "1", text: "hello world", category: "fact", importance: 0.5, timestamp: 1, scope: "user" }],
      0.5,
      2,
    );
  ...
  it("clusters similar entries", () => {
    const entries = [
      { id: "1", text: "hello world foo", category: "fact", importance: 0.8, timestamp: 1, scope: "user" },
      { id: "2", text: "hello world bar", category: "fact", importance: 0.6, timestamp: 2, scope: "user" },
      { id: "3", text: "completely different text here", category: "fact", importance: 0.5, timestamp: 3, scope: "user" },
    ];
...
describe("runTextCompaction", () => {
  it("returns zero result when disabled", () => {
    const result = runTextCompaction([], { enabled: false, minAgeDays: 7, similarityThreshold: 0.5, minClusterSize: 2, maxEntriesToScan: 200, dryRun: false });
  ...
  it("dry run reports clusters without deleting", () => {
    const entries = [
      { id: "1", text: "hello world", category: "fact", importance: 0.5, timestamp: 1, scope: "user" },
      { id: "2", text: "hello world", category: "fact", importance: 0.5, timestamp: 2, scope: "user" },
    ];
    const result = runTextCompaction(entries, { enabled: true, minAgeDays: 0, similarityThreshold: 0.5, minClusterSize: 2, maxEntriesToScan: 200, dryRun: true });
