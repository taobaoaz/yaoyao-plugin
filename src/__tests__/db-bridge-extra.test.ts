...
try {
  ...
  d.close();
  VEC_AVAILABLE = true;
} catch { /* sqlite-vec not installed */ }
...
describe("Hybrid Search", { concurrency: 1 }, () => {
  ...
  it("FTS5 search: empty query handled gracefully", () => {
    try {
      const stmt = db.prepare("SELECT COUNT(*) as c FROM yaoyao_fts WHERE yaoyao_fts MATCH ?");
      stmt.all("");
    } catch {
  ...
  it("FTS5 search: results ordered by rank", () => {
    const stmt = db.prepare(
      ...
      "FROM yaoyao_fts WHERE yaoyao_fts MATCH ? ORDER BY rank LIMIT 10"
    );
  ...
  it("FTS5 search: LIMIT works", () => {
    const stmt = db.prepare(
      "SELECT date, snippet(yaoyao_fts, 2, '<b>', '</b>', '…', 32) as snippet, rank " +
      "FROM yaoyao_fts WHERE yaoyao_fts MATCH ? ORDER BY rank LIMIT ?"
    );
  ...
  it("LIKE search: finds results", () => {
    const stmt = db.prepare("SELECT COUNT(*) as c FROM yaoyao_meta WHERE user_text LIKE ?");
  ...
  it("FTS5 rebuild after insert", () => {
    ...
    const beforeStmt = db.prepare("SELECT COUNT(*) as c FROM yaoyao_fts WHERE yaoyao_fts MATCH ?");
    const beforeResult = beforeStmt.get("test") as { c: number };
    assert.ok(beforeResult.c > 0, "Should find data before rebuild");
    // Rebuild — this syncs content=yaoyao_meta table back into the FTS index
    db.exec("INSERT INTO yaoyao_fts(yaoyao_fts) VALUES('rebuild')");
    const afterStmt = db.prepare("SELECT COUNT(*) as c FROM yaoyao_fts WHERE yaoyao_fts MATCH ?");
    const afterResult = afterStmt.get("test") as { c: number };
    // After rebuild, data from yaoyao_meta should still be searchable
